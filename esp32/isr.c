 ...
 *
 * ESP32 does NOT have ETS_WIFI_PWR_INTR_SOURCE (that's ESP32-S3/C3 only).
 * Source 2 on ESP32 is ETS_WIFI_BB_INTR_SOURCE directly. */
...
/* On Xtensa, interrupt types are determined by hardware, not software-
 * configurable like on RISC-V. No-op for ESP32. */
void espradio_wifi_int_to_level(void) {
}

/* On Xtensa, interrupt priorities are fixed by hardware.
 * No-op for ESP32. */
