// This is examples/http-app with Go heap instrumentation bolted on, for comparing
// the Go GC's behaviour across two builds of the driver.  See gcstats.go for what
// is measured and README.md for the A/B procedure.
//
// The server itself is a verbatim copy of examples/http-app and should stay that
// way: the whole point is that the allocation profile of the workload is identical
// on both sides of the comparison, so any difference in the numbers comes from the
// driver rather than from the example.  Only three statements differ from the
// original -- start the sampler, release its baseline, count a connection -- each
// marked with a "gcstats:" comment.
//
// tinygo flash -target xiao-esp32c3 -ldflags="-X main.ssid=YourSSID -X main.password=YourPassword" -monitor ./examples/http-gc
package main
...
func main() {
	...
	go gcReport() // gcstats: sample the Go heap from before bring-up onwards.
	...
	serverReady = true // gcstats: release the sampler to latch its baseline.
...
func handleConn(conn *tcp.Conn, cs *connState, stack *xnet.StackAsync) {
	defer conn.Close()
	reqCount++ // gcstats: counted here, not after the response, so a connection that
	// allocated and then aborted still appears in the per-request denominator.
